...
if archivo_cargado is not None:
    ...
    # ==========================================
    tab1, tab2, tab3 = st.tabs(["📈 Módulo 5: Dashboard Interactivo", "🧠 Módulo 3: Consultor Predictivo IA", "📬 Módulo 4: Despacho Masivo (PDF)"])
    ...
    # --- TAB 3: DESPACHO MASIVO CON PDF ---
    with tab3:
        st.subheader("Motor de Compilación a PDF y Distribución")
        st.write("Este módulo consolida la data, usa la librería `docx2pdf` para exportar a PDF y despacha por Gmail.")
        
        PLANTILLA = "Plantilla_Reporte.docx"
        if not os.path.exists(PLANTILLA):
            st.error(f"🚨 Recuerda subir el archivo '{PLANTILLA}' a la misma carpeta.")
        else:
            lista_distribucion = {
            ...
            if st.button("🚀 Ejecutar Despacho Masivo en PDF", use_container_width=True):
                if not clave_app:
                    st.error("⚠️ Falta la Contraseña de Aplicación de tu correo en la barra lateral.")
                else:
                    status_bar = st.status("Iniciando motor de conversión y envío...", expanded=True)
                    ...
                    try:
                        for hoja, df_hoja in dict_proyectos.items():
                            ...
                            # 1. Crear Word a partir de la plantilla
                            ...
                            nombre_docx = os.path.abspath(f"Temp_{hoja}_{marca_t}.docx")
                            nombre_pdf = os.path.abspath(f"Reporte_{hoja}_{marca_t}.pdf")
                            doc_template.save(nombre_docx)
                            
                            # 2. Conversión a PDF nativa con docx2pdf
                            status_bar.write(f"🔄 Convirtiendo {hoja} a PDF...")
                            convert(nombre_docx, nombre_pdf)
                            
                            # 3. Envío de Correo con el PDF adjunto
